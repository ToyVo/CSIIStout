...
public:
	Circle2D() : Shape2D(0,0)
	{
		radius = 1;
	}
	Circle2D(double x, double y, double r) : Shape2D(x, y)
	{
		radius = r;
	}
	...
	{
		out<<"Circle, Radius: "<<radius<<" Area: "<<getarea()<<" Location "<<printLocation();
	}
};
...
public:
	...
	{
		out<<"Rectangle, "<<width<<"X"<<height<<" Area: "<<getArea()<<" Location "<<printLocation();
	}
};
