//Will take input from a file and read the data from it to be able to then print out the ascii art in the terminal
#include <stdio.h>
#include <conio.h>
#include <stdlib.h>
#include <string.h>
...
void print(FILE *artFile)
{
	...
	while (fscanf(artFile, "%d%c%d%c%c%c%d",&row, &discard, &column, &discard, &symbol, &discard, &repeat) == 7)
	{
		//checks for if any values are going to go outside of the defined resolution
...
main()
{
	...
	printf("Enter a file name and extention. Make sure that the file is in the correct directory:\n");
	printf("This program includes: fly.art, worry.art, heart.art, seuss.art, impossible.art, cupid.art, box.art, and bigbox.art\n");
	scanf("%s", FileName);
